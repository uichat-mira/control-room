...
export function buildReviewPrompt(pkg: ReviewPackage): ReviewPrompt {
  ...
  const system = [
    ...
    "findings must be a JSON array of finding objects. Use [] when there are no high-confidence P0-P2 findings.",
    ...
    "validationGaps must be a JSON array of non-empty strings only. Never return objects or nested structures inside validationGaps. Do not copy deterministicValidationGaps objects verbatim; summarize each material gap as a string. Use [] when no meaningful validation gap exists.",
